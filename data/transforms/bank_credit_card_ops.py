...
def to_curated_bank_credit_card_counts(
    raw_observations: list[BankCreditCardCountRawObservation],
) -> list[BankCreditCardCountsCuratedObservation]:
    ...
    for observation in raw_observations:
        ...
        row = counts_by_key.setdefault(
            key,
            {
                ...
                "cards_with_operations_primary": Decimal("0"),
                "cards_with_operations_supplementary": Decimal("0"),
            },
        )
    ...
    for (institution_code, period_month), row in sorted(counts_by_key.items()):
        ...
        cards_with_operations_primary = row["cards_with_operations_primary"]
        cards_with_operations_supplementary = row["cards_with_operations_supplementary"]
        total_active_cards = active_cards_primary + active_cards_supplementary
        total_cards_with_operations = (
            cards_with_operations_primary + cards_with_operations_supplementary
        )
        ...
        curated.append(
            BankCreditCardCountsCuratedObservation(
                ...
                total_active_cards=total_active_cards,
                cards_with_operations_primary=cards_with_operations_primary,
                cards_with_operations_supplementary=cards_with_operations_supplementary,
