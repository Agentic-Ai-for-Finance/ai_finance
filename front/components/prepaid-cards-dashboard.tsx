...
import {
  type PrepaidCardMetricRow,
  fetchPrepaidCardMetrics,
  fetchPrepaidDatasetBoundary,
  fetchPrepaidOperationMetricsBoundary,
  fetchPrepaidOperationMetrics,
  type PrepaidOperationMetricRow,
} from "@/lib/supabase-prepaid-queries";
import { fetchLatestUfValue } from "@/lib/supabase-queries";
import {
  type PrepaidCustomerType,
  prepaidChartViews,
  defaultPrepaidOperationsRateViewKey,
  defaultPrepaidViewKey,
  isPrepaidChartViewKey,
  isPrepaidOperationsRateOperation,
  isPrepaidOperationsRateViewKey,
  prepaidOperationLabelMap,
  prepaidOperationMetricsViews,
  type PrepaidChartViewKey,
...
export function PrepaidCardsDashboard({
  customerType,
  operation,
  initialView,
  startMonthParam,
  ...
  const initialMetricKey = isOperationsRateDashboard
    ? isPrepaidOperationsRateViewKey(initialView)
      ? initialView
      : defaultPrepaidOperationsRateViewKey
    : isPrepaidChartViewKey(initialView)
      ? initialView
      : defaultPrepaidViewKey;

  const [viewKey, setViewKey] = useState<PrepaidChartViewKey | PrepaidOperationMetricsViewKey>(initialMetricKey);
  const [boundaryState, setBoundaryState] = useState<BoundaryState | null>(null);
  const [operationRows, setOperationRows] = useState<PrepaidCardMetricRow[]>([]);
  const [operationsRateRows, setOperationsRateRows] = useState<PrepaidOperationMetricRow[]>([]);
      ...
      try {
        if (isOperationsRateDashboard) {
          const [latestMonth, earliestMonth] = await Promise.all([
            fetchPrepaidOperationMetricsBoundary(customerType, "latest"),
            fetchPrepaidOperationMetricsBoundary(customerType, "earliest"),
          ]);
        ...
        if (!latestMonth || !earliestMonth) {
          throw new Error("No prepaid-card data is available for this operation.");
        }
  ...
  }, [customerType, isOperationsRateDashboard, operation]);
  ...
  useEffect(() => {
    ...
    async function loadRows() {
      ...
      try {
        ...
        const nextRows = await fetchPrepaidCardMetrics(
          customerType,
          operation as Exclude<PrepaidOperationName, "Total Activation Rate">,
          `${startMonth}-01`,
          `${endMonth}-01`,
        ...
        if (!isCancelled) {
          setOperationRows([]);
          setOperationsRateRows([]);
          setErrorMessage(error instanceof Error ? error.message : "Unable to load prepaid-card metrics.");
        }
      } finally {
        if (!isCancelled) {
          setIsLoadingRows(false);
  ...
  const activeMetric = isOperationsRateDashboard
    ? prepaidOperationMetricsViews.find((item) => item.key === viewKey) ?? prepaidOperationMetricsViews[0]
    : prepaidChartViews.find((item) => item.key === viewKey) ?? prepaidChartViews[0];

  const filteredOperationRows = useMemo(() => operationRows, [operationRows]);
  const filteredOperationsRateRows = useMemo(() => operationsRateRows, [operationsRateRows]);
    ...
    if (isOperationsRateDashboard) {
      mergedOperationsRateRows.forEach((row) => {
        ...
        const metricValue = getOperationsRateMetricValue(
          row,
          viewKey as PrepaidOperationMetricsViewKey
        );
      ...
      mergedOperationRows.forEach((row) => {
        ...
        const metricValue = getOperationMetricValue(
          row,
          viewKey as PrepaidChartViewKey,
          activeUfValue
        );
        const canonical = getCanonicalInstitution(row.institution_name, row.institution_code);
  ...
  const summaryRows = useMemo(() => {
    ...
    const selectedRows: SummaryRow[] = selectedBanks
      .map((institutionCode) => {
        ...
        const currentValue = isOperationsRateDashboard
          ? getOperationsRateMetricValue(row as PrepaidOperationMetricRow, viewKey as PrepaidOperationMetricsViewKey)
          : getOperationMetricValue(row as PrepaidCardMetricRow, viewKey as PrepaidChartViewKey, activeUfValue);

        const startRow = firstRowsByCode.get(institutionCode);
        const startValue = startRow
          ? isOperationsRateDashboard
            ? getOperationsRateMetricValue(startRow as PrepaidOperationMetricRow, viewKey as PrepaidOperationMetricsViewKey)
            : getOperationMetricValue(startRow as PrepaidCardMetricRow, viewKey as PrepaidChartViewKey, activeUfValue)
          : null;
        ...
        const shareStart = (() => {
          ...
          const systemStartTotal = calculateSystemTotal(
            firstMonthRows,
            viewKey as PrepaidChartViewKey | PrepaidOperationMetricsViewKey,
            activeUfValue,
            isOperationsRateDashboard
          );
          ...
          const institutionStartValue = getOperationMetricValue(
            startRow as PrepaidCardMetricRow,
            viewKey as PrepaidChartViewKey,
            activeUfValue
          );
    ...
    if (isOperationsRateDashboard) {
      const systemCurrentValue = calculateOperationsRateSystemValue(
        latestMonthRows as PrepaidOperationMetricRow[],
        viewKey as PrepaidOperationMetricsViewKey
      );
      const systemStartValue = calculateOperationsRateSystemValue(
        firstMonthRows as PrepaidOperationMetricRow[],
        viewKey as PrepaidOperationMetricsViewKey
      );
    ...
    if (viewKey === "operations-per-active-card") {
      const systemCurrentValue = calculateOperationsPerActiveCardSystemValue(latestMonthRows as PrepaidCardMetricRow[]);
      const systemStartValue = calculateOperationsPerActiveCardSystemValue(firstMonthRows as PrepaidCardMetricRow[]);
    ...
    const selectedStartTotal = selectedBanks.reduce((accumulator, institutionCode) => {
      ...
      const startValue = getOperationMetricValue(startRow as PrepaidCardMetricRow, viewKey as PrepaidChartViewKey, activeUfValue);
    ...
    return [
      {
        ...
        metricGrowthPct: calculateVsStart(
          calculateSystemTotal(firstMonthRows, viewKey as PrepaidChartViewKey | PrepaidOperationMetricsViewKey, activeUfValue, isOperationsRateDashboard),
          totalValue,
          startMonth === latestLoadedMonth
        ),
        marketShareEnd: 100,
      ...
      <div className="border-b border-border pb-6 sm:pb-8">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-4 text-3xl font-semibold tracking-tight text-white sm:mt-5 sm:text-5xl lg:text-6xl">
            {getEditorialTitle(prepaidOperationLabelMap[operation])}
          </h1>
          <p className="mt-4 max-w-none text-sm leading-6 text-muted sm:mt-5 sm:text-base sm:leading-7 lg:text-lg">
            Monthly prepaid-card analysis across issuers, with UF-adjusted CLP values for volume and ticket metrics, plus active-card and activation-rate views.
          </p>
        </div>
      </div>
                  ...
                  description={
                    !isOperationsRateDashboard
                      ? item.key === "average-ticket"
                        ? `Avg ${prepaidOperationLabelMap[operation]} in CLP.`
                        : item.key === "operations-per-active-card"
                          ? `Number of .* per active prepaid card.`
                          : item.description.replace("for the selected operation", `for ${prepaidOperationLabelMap[operation]}`)
                      : item.description
                  }
...
function getOperationMetricValue(
  row: PrepaidCardMetricRow,
  viewKey: PrepaidChartViewKey,
...
function aggregateOperationRows(rows: PrepaidCardMetricRow[]): PrepaidCardMetricRow[] {
  const grouped = new Map<string, PrepaidCardMetricRow>();
...
function aggregateOperationsRateRows(rows: PrepaidOperationMetricRow[]): PrepaidOperationMetricRow[] {
  const grouped = new Map<string, PrepaidOperationMetricRow>();
...
function getOperationsRateMetricValue(
  row: PrepaidOperationMetricRow,
  viewKey: PrepaidOperationMetricsViewKey
): number | null {
...
function calculateSystemAverage(rows: Array<PrepaidCardMetricRow | PrepaidOperationMetricRow>, activeUfValue: number) {
...
function calculateOperationsPerActiveCardSystemValue(rows: PrepaidCardMetricRow[]) {
  const totals = rows.reduce(
    (accumulator, row) => {
      accumulator.transactions += Number(row.transaction_count);
...
function calculateOperationsRateSystemValue(rows: PrepaidOperationMetricRow[], viewKey: PrepaidOperationMetricsViewKey) {
  const totals = rows.reduce(
    (accumulator, row) => {
...
function calculateSystemTotal(
  rows: Array<PrepaidCardMetricRow | PrepaidOperationMetricRow>,
...
function getEditorialTitle(operationLabel: string) {
  if (operationLabel === "Operation Metrics") {
    return "Prepaid-card activation metrics across the system";
  }
