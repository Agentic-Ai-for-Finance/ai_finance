...
class FakeTable:
    ...
    def execute(self):
        ...
        if self.name == BANK_DEBIT_CARD_OPS_CURATED_TABLE:
            return FakeResponse(self.db["curated"] or self.db["latest_curated"])

        if self.name == BANK_DEBIT_CARD_COUNTS_CURATED_TABLE:
...
def _curated_observation():
    return BankDebitCardOpsCuratedObservation(
        operation_type="Debit Transactions",
        dataset_code=BANK_DEBIT_CARD_OPS_DEBIT_TRANSACTIONS_DATASET,
        institution_code="BICE",
        institution_name="Banco BICE",
        ...
        source_dataset_code=BANK_DEBIT_CARD_OPS_DEBIT_TRANSACTIONS_DATASET,
    )


def _count_raw_observation():
    return BankDebitCardCountRawObservation(
        dataset_code="bank_debit_card_active_cards_primary_debit",
...
def _count_curated_observation():
    return BankDebitCardCountsCuratedObservation(
        dataset_code=BANK_DEBIT_CARD_COUNTS_DATASET,
        institution_code="BICE",
        institution_name="Banco BICE",
        period_month=date(2026, 4, 1),
        active_cards_primary=Decimal("125"),
        active_cards_supplementary=Decimal("15"),
        total_active_cards=Decimal("140"),
        total_cards_with_operations=Decimal("95"),
        operations_rate=Decimal("0.6785"),
    )
...
def test_latest_curated_operation_month_returns_none_for_empty_table():
    assert (
        latest_curated_operation_month(
            FakeSupabase(), dataset_code=BANK_DEBIT_CARD_OPS_DEBIT_TRANSACTIONS_DATASET
        )
        is None
    )
...
def test_upsert_bank_debit_card_ops_raw_uses_idempotent_conflict_key():
    sb = FakeSupabase()

    upsert_bank_debit_card_ops_raw(sb, [_raw_observation()])

    assert sb.upserts[0]["table"] == BANK_DEBIT_CARD_OPS_RAW_TABLE
    assert sb.upserts[0]["kwargs"] == {
        "on_conflict": "dataset_code,source_codigo,period_month"
    }


def test_upsert_bank_debit_card_ops_curated_uses_idempotent_conflict_key():
    sb = FakeSupabase()

    upsert_bank_debit_card_ops_curated(sb, [_curated_observation()])

    assert sb.upserts[0]["table"] == BANK_DEBIT_CARD_OPS_CURATED_TABLE
    assert sb.upserts[0]["kwargs"] == {
        "on_conflict": "dataset_code,institution_code,period_month"
    }
    assert sb.upserts[0]["payload"][0]["operations_per_active_card"] == "5"


def test_upsert_bank_debit_card_count_raw_uses_idempotent_conflict_key():
    sb = FakeSupabase()

    upsert_bank_debit_card_count_raw(sb, [_count_raw_observation()])

    assert sb.upserts[0]["table"] == BANK_DEBIT_CARD_COUNTS_RAW_TABLE
    assert sb.upserts[0]["kwargs"] == {
        "on_conflict": "dataset_code,source_codigo,period_month"
    }


def test_upsert_bank_debit_card_counts_curated_uses_idempotent_conflict_key():
    sb = FakeSupabase()

    upsert_bank_debit_card_counts_curated(sb, [_count_curated_observation()])

    assert sb.upserts[0]["table"] == BANK_DEBIT_CARD_COUNTS_CURATED_TABLE
    assert sb.upserts[0]["kwargs"] == {
        "on_conflict": "dataset_code,institution_code,period_month"
    }
    assert sb.upserts[0]["payload"][0]["total_cards_with_operations"] == "95"
